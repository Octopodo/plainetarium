...
const planetOffsetX = computed(
  () => basePlanetOffset.value + (offsetX.value * -basePlanetOffset.value) / 100 //Compute the offset
);
const planetOffsetY = computed(
  () => basePlanetOffset.value + (offsetY.value * -basePlanetOffset.value) / 100 //Compute the offset
);
...
<template>
  <div class="mask overflow-hidden rounded-full">
    <slot />
  </div>
</template>
